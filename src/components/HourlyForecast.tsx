...
const HourlyForecast: React.FC<HourlyForecastProps> = ({ hourly, temperatureUnit, windSpeedUnit }) => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-6 shadow-xl animate-slide-up">
      <h3 className="text-lg md:text-xl font-bold text-gray-800 mb-4">Hourly Forecast</h3>
      <div className="overflow-x-auto pb-2 -mx-4 md:mx-0 px-4 md:px-0">
        <div className="flex space-x-3 md:space-x-4 min-w-max">{next24Hours.map((time, index) => {
            ...
            return (
              <div
                key={time}
                className="flex flex-col items-center bg-gray-50 rounded-2xl p-3 md:p-4 min-w-[70px] md:min-w-[80px] hover:bg-blue-50 transition-colors active:scale-95"
              >
